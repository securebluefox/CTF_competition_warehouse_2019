...
class ypmodel_field extends admin {
	private $db,$model_db;
	public $siteid;
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			if (!$model_table) $tablename = $this->db->db_tablepre.'yp_company';
			...
			$this->cache_field($modelid);
			showmessage(L('add_success'),'?m=yp&c=ypmodel_field&a=init&modelid='.$modelid.'&menuid=59');
		} else {
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			$model_cache = getcache('yp_model','model');
			$modelid = intval($_POST['info']['modelid']);
			$model_table = $model_cache[$modelid]['tablename'];
			...
			if ($field == 'catids' && $_POST['setting']['boxtype']=='multiple') {
				$field_type = 'varchar';
			}
			...
			$this->cache_field($modelid);
			showmessage(L('update_success'),'?m=yp&c=ypmodel_field&a=init&modelid='.$modelid.'&menuid=59');
		} else {
			$show_header = $show_validator = $show_dialog = '';
			pc_base::load_sys_class('form','',0);
			require MODEL_PATH.'fields.inc.php'; 
	...
	public function public_checkfield() {
		...
		if (!$tablename) $tablename = 'yp_company';
