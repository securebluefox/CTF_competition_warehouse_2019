...
<div class="subnav">
  <h2 class="title-1 line-x f14 fb blue lh28"><?php echo L('model_manage');?>--<?php echo L('additional').L('field_manage');?></h2>
<div class="content-menu ib-a blue line-x"><a class="add fb" href="?m=yp&c=additional&a=add&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('add_additional_field')?></em></a>
　<a class="on" href="?m=yp&c=additional&a=init&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('manage_additional_field')?></em></a><span>
</div></div>
<div class="pad-lr-10">
<form name="myform" action="?m=yp&c=additional&a=listorder" method="post">
<div class="table-list">
    ...
    <tr>
		...
		<td align='center'> <a href="?m=yp&c=additional&a=edit&fieldid=<?php echo $r['fieldid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo L('edit');?></a> |
		<?php if(!in_array($r['field'],$forbid_fields)) { ?>
		<a href="?m=yp&c=additional&a=disabled&disabled=<?php echo $r['disabled'];?>&fieldid=<?php echo $r['fieldid']?>&fieldid=<?php echo $r['fieldid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo $r['disabled'] ? L('field_enabled') : L('field_disabled');?></a>
		<?php } else { ?><font color="#BEBEBE"> <?php echo L('field_disabled');?> </font><?php } ?>|<?php if(!in_array($r['field'],$forbid_delete)) {?>
		<a href="javascript:confirmurl('?m=yp&c=additional&a=delete&fieldid=<?php echo $r['fieldid']?>&menuid=<?php echo $_GET['menuid']?>','<?php echo L('confirm',array('message'=>$r['name']))?>')"><?php echo L('delete')?></a><?php } else {?><font color="#BEBEBE"> <?php echo L('delete');?></font><?php }?> </td>
	</tr>
	<?php } ?>
    </tbody>
    </table>
