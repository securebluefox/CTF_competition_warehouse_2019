...
//        if(!$this->session->userdata('login')){redirect(base_url().'index.php/login');}
    ...
    public function index_get($action='')
    {
        if($action===''){
            //var_dump($_SESSION);
            if(!$this->session->userdata('login')){header("Location: ./login");exit();}
            $user = new stdClass();
        ...
        if($action==='change'){
            ...
            $this->load->view('Pass_change' , $data);
        }
    }
    ...
    public function index_post($action='')
    {
        ...
        if($action==='change'){
            ...
            if($password === $password_confirm){
                if($user->password === hash("sha256", $old_password)){
                    $user_data = [
                        'password' => hash("sha256", $password),
                    ];
                    $this->db->where('username', $this->session->userdata('username'))->update('user', $user_data);
                    $data['success'] = 1;
                    $data['token_name'] = $this->security->get_csrf_token_name();
                    $data['token_hash'] = $this->security->get_csrf_hash();
