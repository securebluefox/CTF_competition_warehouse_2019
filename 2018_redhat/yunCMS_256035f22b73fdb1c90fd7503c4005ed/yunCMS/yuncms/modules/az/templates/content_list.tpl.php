...
<!--
	...
	parent.document.getElementById('center_frame').src = '?m=yp&c=content&a=public_categorys&modelid=<?php echo $modelid?>&type=add&menuid=<?php echo $_GET['menuid'];?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>';
...
function reset_click() {
	window.top.$("#current_pos").data('clicknum',1);
}
//-->
</SCRIPT>
<div class="subnav">
  <h2 class="title-1 line-x f14 fb blue lh28"><?php echo L('yp_menu')?></h2>
<div class="content-menu ib-a blue line-x">
　<?php foreach ($yp_model as $m) { ?><a<?php if ($modelid==$m['modelid']){?> class="on" href="javascript:void(0);"<?php } else {?> href="?m=yp&c=content&modelid=<?php echo $m['modelid']?>" onclick="reset_click();" <?php }?>><em><?php echo $m['name']?><?php echo L('manage_content')?></em></a><span>|</span><?php }?><a href="?m=yp&c=content&a=init&catid=<?php echo $catid?>&status=99&modelid=<?php echo $modelid?>&catid=<?php echo $catid;?>&pc_hash=<?php echo $pc_hash;?>" <?php if($status==99) echo 'class=on';?>><em><?php echo L('check_passed');?></em></a><span>|</span><a href="?m=yp&c=content&a=init&catid=<?php echo $catid?>&status=1&modelid=<?php echo $modeid?>" <?php if($status==1 || !isset($status)) echo 'class=on';?>><em><?php echo L('checked')?></em></a><span>|</span>
...
<input type="hidden" value="yp" name="m">
...
<tbody>
	...
	if(is_array($datas)) {
 	...
 	?>
		...
		if($status==99) {
			...
			echo '<a href="javascript:;" onclick=\'window.open("?m=yp&c=content&a=public_preview&modelid='.$modelid.'&catid='.$r['catid'].'&id='.$r['id'].'","manage")\'>';
		}?><span<?php echo title_style($r['style'])?>><?php echo $r['title'];?></span></a> <?php if($r['thumb']!='') {echo '<img src="'.IMG_PATH.'icon/small_img.gif" title="'.L('thumb', '', 'content').'">'; } if($r['posids']) {echo '<img src="'.IMG_PATH.'icon/small_elite.gif" title="'.L('elite').'">';} if($r['islink']) {echo ' <img src="'.IMG_PATH.'icon/link.png" title="'.L('islink_url').'">';}?></td>
		...
		<td align='center'><a href="javascript:;" onclick="javascript:openwinx('?m=yp&c=content&a=edit&modelid=<?php echo $modelid?>&catid=<?php echo $r[catid];?>&id=<?php echo $r['id']?>','')"><?php echo L('edit');?></a> </td>
	</tr>
     <?php }
	}
	?>
    ...
    <div class="btn"><label for="check_box"><?php echo L('selected_all');?>/<?php echo L('cancel');?></label>
		<input type="hidden" value="<?php echo $pc_hash;?>" name="pc_hash">
    	<input type="button" class="button" value="<?php echo L('listorder');?>" onclick="myform.action='?m=yp&c=content&a=listorder&dosubmit=1&modelid=<?php echo $modelid;?>';myform.submit();"/>
		<?php if($status!=99) {?>
		<input type="button" class="button" value="<?php echo L('passed_checked');?>" onclick="myform.action='?m=yp&c=content&a=pass&modelid=<?php echo $modelid;?>';myform.submit();"/>
		<?php }?>
		<input type="button" class="button" value="<?php echo L('delete');?>" onclick="myform.action='?m=yp&c=content&a=delete&dosubmit=1&modelid=<?php echo $modelid;?>';return confirm_delete()"/>
...
function push() {
	...
	window.top.art.dialog({title:'<?php echo L('push');?>：',id:'push',iframe:'?m=yp&c=push&action=position_list&catid=<?php echo $catid?>&modelid=<?php echo $modelid?>&id='+id,width:'800',height:'500'}, function(){var d = window.top.art.dialog({id:'push'}).data.iframe;// 使用内置接口获取iframe对象
...
function reject_check(type) {
	if(type==1) {
		...
		document.getElementById('myform').action='?m=yp&c=content&a=pass&modelid=<?php echo $modelid;?>&reject=1';
