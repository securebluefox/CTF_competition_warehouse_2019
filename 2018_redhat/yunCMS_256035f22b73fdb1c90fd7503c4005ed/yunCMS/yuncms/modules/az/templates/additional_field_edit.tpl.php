...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('fieldname_was_wrong');?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "m=yp&c=additional&a=public_checkfield&oldfield=<?php echo $field;?>",
		datatype : "html",
		cached:false,
...
<a href="javascript:;" class="on"><em><?php echo L('edit_field');?></em></a><span>|</span><a href="?m=yp&c=additional&a=init&modelid=<?php echo $modelid?>&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('manage_field');?></em></a><span>|</span></div>
  <div class="bk10"></div>
</div>
<form name="myform" id="myform" action="?m=yp&c=additional&a=edit" method="post">
...
<option value="/^[0-9]{6}$/"><?php echo L('zip');?></option>
    ...
    <input type="hidden" name="info[isadd]" value="1">
	<tr> 
