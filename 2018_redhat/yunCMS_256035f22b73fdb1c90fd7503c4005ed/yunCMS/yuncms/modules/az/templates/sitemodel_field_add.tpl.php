...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('fieldname_was_wrong');?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "m=yp&c=ypmodel_field&a=public_checkfield&modelid=<?php echo $modelid?>",
		datatype : "html",
		cached:false,
...
　<a href="?m=yp&c=ypmodel_field&a=init&modelid=<?php echo $modelid?>&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('manage_field');?></em></a><span>|</span></div>
  <div class="bk10"></div>
</div>
<form name="myform" id="myform" action="?m=yp&c=ypmodel_field&a=add" method="post">
...
<?php echo form::select($all_field,'','name="info[formtype]" id="formtype" onchange="javascript:field_setting(this.value);"',L('select_fieldtype'));?>
    ...
    </tr><?php } elseif ($m_r['type']==4) {?><input type="hidden" name="issystem" id="issystem" value="1"><input type="hidden" name="info[issystem]" value="1"><?php }?>
	<tr> 
