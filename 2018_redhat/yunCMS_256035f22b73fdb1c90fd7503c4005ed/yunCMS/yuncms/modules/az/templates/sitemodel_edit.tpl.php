...
<div class="pad-lr-10">
<form action="?m=yp&c=ypmodel&a=edit" method="post" id="myform">
<fieldset>
  ...
  <tr>
    ...
    <td class="y-bg"><input type='radio' id="show_menu" name='setting[ismenu]' value='1'<?php if ($setting['ismenu']) {?> checked<?php }?>> 是&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type='radio' name='setting[ismenu]' id="hide_menu" value='0'<?php if ($setting['ismenu']==0) {?> checked<?php }?>> 否</td>
  </tr>
</table>
</fieldset>
<fieldset id="template_show"<?php if ($setting['ismenu']==0) {?> style="display:none;"<?php }?>>
	<legend><?php echo L('template_setting')?></legend>
	<table width="100%"  class="table_form">
  <tr>
  <th width="120"><?php echo L('available_styles');?></th>
        <td>
		<?php echo form::select($style_list, $default_style, 'name="info[default_style]" id="default_style" onchange="load_file_list(this.value)"', L('please_select'))?>
		</td>
</tr>
		<tr>
        <th width="120"><?php echo L('category_index_tpl')?>：</th>
        <td  id="model_template"><?php if ($default_style) echo '<script type="text/javascript">$.getJSON(\'?m=admin&c=category&a=public_tpl_file_list&style='.$default_style.'&id='.$category_template.'|'.$list_template.'|'.$show_template.'&module=yp&templates=model|list|show&name=info&pc_hash=\'+pc_hash, function(data){$(\'#model_template\').html(data.model_template);$(\'#list_template\').html(data.list_template);$(\'#show_template\').html(data.show_template);});</script>';?>
		</td>
      </tr>
	  <tr>
