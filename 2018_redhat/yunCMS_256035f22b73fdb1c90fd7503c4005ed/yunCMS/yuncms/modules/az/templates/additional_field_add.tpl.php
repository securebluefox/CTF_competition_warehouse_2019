...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('fieldname_was_wrong');?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "m=yp&c=additional&a=public_checkfield",
		datatype : "html",
		cached:false,
...
　<a href="?m=yp&c=additional&a=init&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('manage_additional_field')?></em></a></div>
  <div class="bk10"></div>
</div>
<form name="myform" id="myform" action="?m=yp&c=additional&a=add" method="post">
...
<option value="/^[0-9]{6}$/"><?php echo L('zip');?></option>
	...
	<input type="hidden" name="info[isunique]" value="0"> <input type="hidden" name="info[isadd]" value="1">
	<tr>
      <th><strong><?php echo L('basic_field');?></strong><br /><?php echo L('separate_field_in_details_page')?></th>
...
<script type="text/javascript">
	...
	$.each( ['<?php echo implode("','",$att_css_js);?>'], function(i, n){
		...
		$.getJSON("?m=yp&c=ypmodel_field&a=public_field_setting&fieldtype="+fieldtype, function(data){
