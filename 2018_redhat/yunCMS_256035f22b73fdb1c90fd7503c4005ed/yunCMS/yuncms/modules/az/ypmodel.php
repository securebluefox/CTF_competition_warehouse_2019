...
class ypmodel extends admin {
	...
	public function init() {
		$categorys = getcache('category_content_yp','yp');

		$datas = $this->db->listinfo("`type` IN ('4', '5')",'',$_GET['page'],30);
		//模型文章数array('模型id'=>数量);
		$items = array();
		foreach ($datas as $k=>$r) {
			...
			$datas[$k]['tablename'] = str_replace('yp_', '', $r['tablename']);
		...
		$this->public_cache();
		$show_header = true;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=yp&c=ypmodel&a=add\', title:\''.L('add_model').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_model'));
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$_POST['info']['type'] = 5;
			$_POST['info']['tablename'] = $_POST['info']['tablename'] ? 'yp_'.$_POST['info']['tablename'] : showmessage(L('table_name_empty'));
			if ($_POST['info']['model_template']) $_POST['info']['category_template'] = $_POST['info']['model_template'];
			unset($_POST['info']['model_template']);
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$tablename = str_replace('yp_', '', $tablename);
	...
	public function delete() {
		...
		$modelid = intval($_GET['modelid']);
		$model_cache = getcache('yp_model','model');
	...
	public function public_cache() {
		...
		//更新模型数据缓存
		$model_array = array();
		$datas = $this->db->select(array('type'=>5));
		foreach ($datas as $r) {
	...
	//黄页模型Rewrite规则
	public function public_rewriteurl() {
		$yp_model = getcache('yp_model', 'model');
		$modelids = array_keys($yp_model);
		$apache_rewriteurl  = $iis_rewriteurl = '';
		if(is_array($modelids)) foreach($modelids as $m) {
			$setting = string2array($yp_model[$m]['setting']);
			if ($setting['ismenu'] == 0) continue;
			$rewriteurl_1 = 'yp-list-([0-9]+)-(.*)-';
			$iis_rewriteurl_1 = '(.*)/yp\-list\-([0-9]+)\-(.*)\-';
			$rewriteurl_2 = 'index.php?m=yp&c=index&a=lists&modelid=$1';
			$iis_rewriteurl_2 = '$1/index\.php\?m=yp&c=index&a=lists&modelid=$2';
			$num = 2;
			...
			if(is_array($fields) && !empty($fields)) {
				...
				foreach ($fields as $_v=>$_k) {
					if($_k['filtertype'] || $_k['rangetype']) {
						...
						$iis_rewriteurl_1 .= '(.*)\-';
						$rewriteurl_2 .= '&'.$_v.'=$'.$num ;
						$iis_rewriteurl_2 .= '&'.$_v.'=$'.intval($num+1) ;
						$num ++;
					}
				}
				$rewriteurl_1 .= '([0-9]+).html';
				$iis_rewriteurl_1 .= '([0-9]+)\.html\?*(.*)$';
				$rewriteurl_2 .= '&page=$'.$num;
				$iis_rewriteurl_2 .= '&page=$'.$num;
				$apache_rewriteurl .= 'RewriteRule ^'.$rewriteurl_1.'   '.$rewriteurl_2."\n";
				$iis_rewriteurl .= 'RewriteRule ^'.$iis_rewriteurl_1.'   '.$iis_rewriteurl_2."\n";
			}
		}
		$ng_rewriteurl .= "if (!-e \$request_filename) {\n\treturn 404;\n}";
		$show_header = 1;
		include $this->admin_tpl('rewriteurl');
	}
	/**
	 * 导出模型
	 */
	function export() {
		...
		$modelarr = getcache('yp_model', 'model');
		//定义系统字段排除
	...
	function import(){
		if(isset($_POST['dosubmit'])) {
			...
			if($is_exists) showmessage(L('operation_failure'),'?m=yp&c=ypmodel&a=init');
			...
			if($modelid){
				...
				$this->public_cache();
				showmessage(L('operation_success'),'?m=yp&c=ypmodel&a=init');
			}
		} else {
			pc_base::load_sys_class('form','',0);
			$show_validator = '';
			...
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=yp&c=ypmodel&a=add\', title:\''.L('add_model').'\', width:\'580\', height:\'400\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_model'));
	...
	public function public_check_tablename() {
		if ($_GET['tablename']) {
			$tablename = 'yp_'.$_GET['tablename'];
		} else {
			exit('0');
		}
		$r = $this->db->table_exists(strip_tags($tablename));
		if(!$r) echo '1';
	}
	/**
	 * 更新指定模型字段缓存
