...
class Application(tornado.web.Application):
    ...
    def _get_ans(self, uuid):
        answer = {}
        with open(os.path.join(self.ans_path, ('ans%s.txt' % uuid).replace("ans.","ans").replace("ans_","ans")), 'r') as f:
    ...
    def _generate_captcha(self):
        ...
        for file in self.files:
            if file.replace('ques', '').replace('.jpg', '').replace(".","")[0]=="_":
                uuids.append(file.replace('ques', '').replace('.jpg', '').replace(".", "")[1:])
            else:
                uuids.append(file.replace('ques', '').replace('.jpg', '').replace(".",""))
